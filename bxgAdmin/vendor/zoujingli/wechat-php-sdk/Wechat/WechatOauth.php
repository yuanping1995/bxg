...
class WechatOauth extends Common {
	 ...
	 * @return string
	 */
	public function getOauth($callback, $state = '', $scope = 'snsapi_login') {
		//Oauth 标准参数
		$redirect_uri = urlencode($callback);
		$url = self::OAUTH_URL . "?appid={$this->appid}&redirect_uri={$redirect_uri}&response_type=code&scope={$scope}&state={$state}#wechat_redirect";
		header("location:$url");
		exit;
	}
	/**
