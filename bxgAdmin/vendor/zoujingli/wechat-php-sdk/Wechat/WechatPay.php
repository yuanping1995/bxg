...
class WechatPay {
	 ...
	 * @return array|bool
	 */
	public function createMicroPay($body,$auth_code, $out_trade_no, $total_fee, $attach, $goods_tag = null) {
		$data = array(
			"appid" => $this->appid,
			"mch_id" => $this->mch_id,
			"body" => $body,
	 ...
	 * @return bool|string
	 */
	public function getPrepayId($openid, $body, $out_trade_no, $total_fee, $notify_url,$attach, $trade_type = "JSAPI", $goods_tag = null, $fee_type = 'CNY') {
		$postdata = array(
			"body" => $body,
			"out_trade_no" => $out_trade_no,
			"fee_type" => $fee_type,
	 ...
	 * @return bool|string
	 */
	public function getQrcPrepayId($openid, $body, $out_trade_no, $total_fee, $notify_url,$attach, $goods_tag = null, $fee_type = 'CNY') {
		$postdata = array(
			"body" => $body,
			"out_trade_no" => $out_trade_no,
			"fee_type" => $fee_type,
	 ...
	 * @return string
	 */
	public function getPayUrl($body, $out_trade_no, $total_fee, $notify_url,$attach,$wap_url,$wap_name) {
		$postdata = array(
			"body" => $body,
			"out_trade_no" => $out_trade_no,
			"total_fee"        => $total_fee,
			...
			"trade_type"     => 'MWEB',
			"spbill_create_ip" => Tools::getAddress(),
			"scene_info"     =>'{"h5_info": {"type": "wap", "wap_url": $wap_url,"wap_name": $wap_name}}'  //上报支付的场景信息
		);
		$result = $this->getArrayResult($postdata, self::MCH_BASE_URL . '/pay/unifiedorder');
		if (false === $this->_parseResult($result)) {
			return false;
	 ...
	 */
	public static function reverse($out_trade_no) {
		$data = array('out_trade_no' => $out_trade_no);
		$result = $this->getArrayResult($data, self::MCH_BASE_URL . '/pay/reverse');
		if (false === $this->_parseResult($result)) {
			return false;
		}
		return $result;
